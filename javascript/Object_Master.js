const pokémon = Object.freeze([
    ...
    const pokémonType=pokémon.filter(pokemon=>pokemon.types=="fire");
    console.log(pokémonType);
    ...
    const pokémonNamesWithIdGreaterThan99 = pokémon.filter(pokemon => pokemon.id > 99).map(pokemon => pokemon.name);
    console.log(pokémonNamesWithIdGreaterThan99);

    // an array with just the names of the pokémon whose only type is poison
    const pokémonNamesWithOnlyPoisonType = pokémon.filter(pokemon => pokemon.
        types.length === 1 && pokemon.types[0] === "poison").map(pokemon => pokemon.name);

    console.log(pokémonNamesWithOnlyPoisonType);

    // an array containing just the first type of all the pokémon whose second type is "flying"

    const firstTypeOfFlyingPokémon = pokémon
    .filter(pokemon => pokemon.types.length === 2 && pokemon.types[1] === "flying")
    .map(pokemon => pokemon.types[0]);
    console.log(firstTypeOfFlyingPokémon);

    // a count of the number of pokémon that are "normal" type
    const normalTypePokémonCount = pokémon.filter(pokemon => pokemon.types.includes("normal")).length;
